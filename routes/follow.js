...
router.put('/task', auth ,async (req, res) => {
  ...
  //<--- friends push
    const userfollowA = await User.find({ name: req.body.name });
        if (userfollowA[0].follows.includes(req.body.friends_name)) { console.log( 'Already a friend!'); }
		else {
        ...
        //---->
		if (createUserA[0].friends.includes(req.body.friends_name)) { res.status(200).json({ message: 'Already a friend!' }); }
		else {
